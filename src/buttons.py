...
class Button():
    ...
    def update(self, screen):
        """vykreslení tlačítka na obrazovku"""
        if self.image is not None:
            screen.blit(self.image, self.rect)
        screen.blit(self.text, self.text_rect)
    
    def check_input(self, position):
        """kontrola, zda se kurzor nachází nad tlačítkem"""
        if position[0] in range(self.rect.left, self.rect.right) and position[1] in range(self.rect.top, self.rect.bottom):
            return True
        return False
    
    def change_color(self, position):
        """změna barvy a obrázku tlačítka při najetí myší"""
        if position[0] in range(self.rect.left, self.rect.right) and position[1] in range(self.rect.top, self.rect.bottom):
    ...
    def click_button(self):
        """kliknutí na talčítko, změna barvy a obrázku"""
        if self.click_status:
            self.text = self.font.render(self.text_input, True, self.clicked_color)
            if self.old_image is not None:
                self.image = self.old_image
    ...
    def reset_click_button(self):
        """vrácení tlačítka do původního stavu, odkliknutí"""
        if self.click_status:
            self.text = self.font.render(self.text_input, True, self.clicked_color)
            if self.old_image is not None:
                self.image = self.old_image
